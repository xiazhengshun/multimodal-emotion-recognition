...
class BaseModel(nn.Module):
    def __init__(self, input_dim=768, output_dim=4):
        super().__init__()
        self.pre_net = nn.Linear(input_dim, 128)

        self.post_net = nn.Linear(128 * 6, output_dim)
...
def validate_and_test(model, data_loader, device, num_classes):
    ...
    with torch.no_grad():
        ...
        for batch in data_loader:
            ...
            ids, net_input1, net_input2, net_input3, net_input4, net_input5, net_input , labels = batch["id"], batch["net_input1"], batch["net_input2"], \
                                                                    batch["net_input3"],  batch["net_input4"], batch["net_input5"],  batch["net_input"], batch["labels"]
            ...
            feats4 = net_input4["feats4"]
            speech_padding_mask4 = net_input4["padding_mask4"]
            feats5 = net_input5["feats5"]
            speech_padding_mask5 = net_input5["padding_mask5"]
            ...
            feats4 = feats4.to(device)
            speech_padding_mask4 = speech_padding_mask4.to(device)
            feats5 = feats5.to(device)
            speech_padding_mask5 = speech_padding_mask5.to(device)
        
            labels = labels.to(device)

            outputs = model(feats1, speech_padding_mask1, feats2, speech_padding_mask2 , feats3, speech_padding_mask3, feats4, speech_padding_mask4, feats5, speech_padding_mask5, feats, speech_padding_mask)
