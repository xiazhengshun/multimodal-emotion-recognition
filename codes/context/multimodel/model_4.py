...
class BertClassifier(nn.Module):
     ...
     def forward(self, upinput_id1, upmask1, upinput_id2, upmask2, upinput_id3, upmask3, upinput_id4, upmask4, input_id, mask):
         # 上文，文本1
         ...
         hidden5, pooled4 = self.bert(input_id, attention_mask=mask, return_dict=False)
         ...
         out = torch.concat((out4, out3, out2, out1, out5), dim= 1)
         out = self.actfc(out)
         #out = self.fc2(out)
         return out
...
class BaseModel(nn.Module):
    ...
    def forward(self, x1, padding_mask1, x2, padding_mask2, x3, padding_mask3, x4, padding_mask4, x, padding_mask):
        ...
        x = torch.concat((x4, x3, x2, x1, x), dim =1)
        #x = self.post_net(x)
        return x
...
class Multimodel(nn.Module):
    def __init__(self, output_dim=4):
        ...
        self.output_layer = nn.Linear(64 * 5, output_dim)

    def forward(self, speech1, padding_mask1, texts1, textsmask1, speech2, padding_mask2, texts2, textsmask2, speech3 ,\
                padding_mask3, texts3, textsmask3, speech4, padding_mask4, texts4, textsmask4, speech, padding_mask, texts, textsmask):
        
        speech_data = self.speechmodel(speech1 ,padding_mask1, speech2 ,padding_mask2, speech3 ,padding_mask3, speech4, padding_mask4, speech, padding_mask)
        texts_data  = self.textmodel(texts1, textsmask1, texts2, textsmask2, texts3, textsmask3, texts4, textsmask4, texts, textsmask)
