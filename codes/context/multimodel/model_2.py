...
class BertClassifier(nn.Module):
    ...
    def __init__(self):
        ...
        self.actfc = nn.GELU()
        self.fc2 = nn.Linear(64 * 3, 4)
         
    def forward(self, upinput_id1, upmask1, upinput_id2, upmask2, input_id, mask):
         # 上文，文本1
        ...
        hidden3, pooled3= self.bert(input_id, attention_mask=mask, return_dict=False)
        ...
        out = torch.concat((out2, out1, out3), dim= 1)
        out = self.actfc(out)
         #out = self.fc2(out)
        return out
...
def validate_and_test(model, data_loader, device, num_classes):
    ...
    with torch.no_grad():
        ...
        for batch in data_loader:
            ids, net_input, net_input1, net_input2, labels = batch["id"], batch["net_input"], batch["net_input1"], batch["net_input2"], batch["labels"]
            ...
            outputs = model(feat1, padmask1, text1, mask1, feat2, padmask2, text2, mask2, feat, padmask, text, mask)
...
class BaseModel(nn.Module):
    def __init__(self, input_dim=768, output_dim=4):
        ...
        self.post_net = nn.Linear(64 * 3, output_dim)
        
        self.activate = nn.GELU()

    def forward(self, x1, padding_mask1, x2, padding_mask2, x, padding_mask):
...
class Multimodel(nn.Module):
    ...
    def forward(self, speech1, padding_mask1, texts1, textsmask1, speech2, padding_mask2, \
                    texts2, textsmask2, speech, padding_mask, texts, textsmask):
        
        speech_data = self.speechmodel(speech1 ,padding_mask1, speech2 ,padding_mask2, speech, padding_mask)
        texts_data  = self.textmodel(texts1, textsmask1, texts2, textsmask2, texts, textsmask)
